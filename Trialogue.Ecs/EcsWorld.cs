...
public class EcsWorld
{
...
#if DEBUG
    internal readonly List<IEcsWorldDebugListener> DebugListeners = new(4);
    private readonly EcsGrowList<EcsEntity> _leakedEntities = new(256);
    ...
    public virtual void Destroy()
    {
        ...
        for (int i = 0, iMax = Filters.Count; i < iMax; i++) Filters.Items[i].Destroy();

        IsDestroyed = true;
#if DEBUG
        for (var i = DebugListeners.Count - 1; i >= 0; i--) DebugListeners[i].OnWorldDestroyed(this);
#endif
    }
    ...
    public EcsEntity NewEntity(string name)
    {
        ...
        foreach (var debugListener in DebugListeners) debugListener.OnEntityCreated(entity);
#endif

        entity.Get<ComponentInfo>().Name = name;
    ...
    public EcsFilter GetFilter(Type filterType, bool createIfNotExists = true)
    {
        ...
        // create new filter.
        var filter = (EcsFilter) Activator.CreateInstance(filterType, BindingFlags.Public | BindingFlags.Instance,
            null, _filterCtor, CultureInfo.InvariantCulture);
#if DEBUG
...
#if DEBUG
        foreach (var debugListener in DebugListeners) debugListener.OnFilterCreated(filter);
#endif
                ...
                for (int i = 0, iMax = filters.Count; i < iMax; i++)
                    if (filters.Items[i].IsCompatible(entityData, 0))
                    {
                        ...
                        if (!filters.Items[i].GetInternalEntitiesMap()
                                .TryGetValue(entity.GetInternalId(), out var filterIdx)) filterIdx = -1;
    ...
    public int GetAllEntities(ref EcsEntity[] entities)
    {
        ...
        if (entities == null || entities.Length < count) entities = new EcsEntity[count];
        EcsEntity e = default;
        e.Owner = this;
        var id = 0;
        for (int i = 0, iMax = EntitiesCount; i < iMax; i++)
