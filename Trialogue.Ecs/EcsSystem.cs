...
/// </summary>
public interface IEcsBeforeInitialiseSystem : IEcsSystem
{
    void OnPreInitialise();
}
...
#endif
public sealed class EcsSystems : IEcsInitialiseSystem, IEcsDestroySystem, IEcsUpdateSystem
{
    public readonly string Name;
    public readonly EcsWorld World;
    private readonly EcsGrowList<IEcsSystem> _allSystems = new(64);
    private readonly EcsGrowList<EcsSystemsUpdateItem> _runSystems = new(64);
    private readonly Dictionary<int, int> _namedRunSystems = new(64);
    private readonly Dictionary<Type, object> _injections = new(32);
    private bool _injected;
#if DEBUG
    private bool _initialized;
    private bool _destroyed;
    private readonly List<IEcsSystemsDebugListener> _debugListeners = new(4);
    ...
    {
        var system = services.GetService<TSystem>();
        system ??= ActivatorUtilities.CreateInstance<TSystem>(services);
        ...
        if (!string.IsNullOrEmpty(namedSystem) && !(system is IEcsUpdateSystem))
            throw new Exception($"Cant name non-{nameof(IEcsUpdateSystem)}.");
#endif
        _allSystems.Add(system);
        if (system is IEcsUpdateSystem)
        {
            if (namedSystem == null && system is EcsSystems ecsSystems) namedSystem = ecsSystems.Name;
            if (namedSystem != null)
            {
#if DEBUG
    ...
    public EcsSystems ProcessInjects()
    {
        ...
        if (!_injected)
        {
            ...
            for (int i = 0, iMax = _allSystems.Count; i < iMax; i++)
                ...
                {
                    foreach (var pair in _injections) nestedSystems._injections[pair.Key] = pair.Value;
    ...
    public static void InjectDataToSystem(IEcsSystem system, EcsWorld world, Dictionary<Type, object> injections)
    {
        ...
        foreach (var f in
                 systemType.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
        {
            // skip statics or fields with [EcsIgnoreInject] attribute.
            if (f.IsStatic || Attribute.IsDefined(f, ignoreType)) continue;
            // EcsWorld
...
#endif
            if (f.FieldType.IsSubclassOf(filterType)) f.SetValue(system, world.GetFilter(f.FieldType));
        }
    }
    ...
    public void OnInitialise()
    {
        ...
        for (int i = 0, iMax = _allSystems.Count; i < iMax; i++)
        {
            ...
            if (system is IEcsBeforeInitialiseSystem preInitSystem)
            {
                preInitSystem.OnPreInitialise();
#if DEBUG
        ...
        {
            var system = _allSystems.Items[i];
            if (system is IEcsInitialiseSystem initSystem)
            {
                initSystem.OnInitialise();
#if DEBUG
    ...
    public void OnUpdate(ref Context context)
    {
        ...
        for (int i = 0, iMax = _runSystems.Count; i < iMax; i++)
        {
            ...
            if (runItem.Active) runItem.System.OnUpdate(ref context);
#if DEBUG
    ...
    public void OnDestroy()
    {
        ...
        {
            var system = _allSystems.Items[i];
            if (system is IEcsAfterDestroySystem postDestroySystem)
            {
                postDestroySystem.OnAfterDestroy();
#if DEBUG
...
#if DEBUG
        for (int i = 0, iMax = _debugListeners.Count; i < iMax; i++) _debugListeners[i].OnSystemsDestroyed(this);
#endif
    }
}
...
internal sealed class RemoveOneFrame<T> : IEcsUpdateSystem where T : struct, IEcsComponent
{
    private readonly EcsFilter<T> _oneFrames = null;

    void IEcsUpdateSystem.OnUpdate(ref Context context)
    {
        for (var idx = _oneFrames.GetEntitiesCount() - 1; idx >= 0; idx--) _oneFrames.GetEntity(idx).Del<T>();
    }
}
