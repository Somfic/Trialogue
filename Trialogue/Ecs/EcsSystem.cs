...
{
    void OnInitialise(ref Context context);
}
...
{
    void OnPostDestroy(ref Context context);
}
...
{
    void OnDestroy(ref Context context);
}
...
{
    void OnRender(ref Context context);
}
...
public sealed class EcsSystems : IEcsStartSystem, IEcsDestroySystem, IEcsUpdateSystem
{
    ...
    private readonly EcsGrowList<EcsSystemsRenderItem> _renderSystems = new(64);
    private readonly Dictionary<int, int> _namedRunSystems = new(64);
    private readonly Dictionary<int, int> _namedRenderSystems = new(64);
    ...
    public EcsSystems Add<TSystem>(IServiceProvider services, string namedSystem = null) where TSystem : IEcsSystem
    {
        ...
        if (!string.IsNullOrEmpty(namedSystem) && !(system is IEcsUpdateSystem || system is IEcsRenderSystem))
            throw new Exception("Cant name non-IEcsRunSystem or non-IEcsRenderSystem.");
#endif
    ...
    public void OnRender(ref Context context)
    {
        ...
        for (int i = 0, iMax = _renderSystems.Count; i < iMax; i++)
        {
            ...
            if (runItem.Active) runItem.System.OnRender(ref context);
#if DEBUG
            if (World.CheckForLeakedEntities(null))
                throw new Exception(
                    $"Empty entity detected, possible memory leak in {_renderSystems.Items[i].GetType().Name}.Run ()");
#endif
        }
    }
