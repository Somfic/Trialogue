...
public sealed class EcsComponentPool<T> : IEcsComponentPool where T : struct, IEcsComponent
{
    ...
    {
        for (int i = 0, iMax = _resizeListenersCount; i < iMax; i++) _resizeListeners[i].OnComponentPoolResize();
    }
    ...
    public void RemoveResizeListener(IEcsComponentPoolResizeListener listener)
    {
        ...
        for (int i = 0, iMax = _resizeListenersCount; i < iMax; i++)
            if (_resizeListeners[i] == listener)
            {
                ...
                if (i < _resizeListenersCount) _resizeListeners[i] = _resizeListeners[_resizeListenersCount];
